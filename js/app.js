'use strict';
const ATLAS_IMG_V = '22'; // кеш-бастер для atlas/*.png|jpg — підняти після заміни картинок
...
function route() {
  ...
  setTitle({ today: 'Сьогодні', progress: 'Мій прогрес', my: 'Мої схеми', edit: 'Редагування', about: 'Про тренажер', decks: 'Колоди', blitz: 'Бліц', course: 'Курс', choose: 'Вибір курсу', krok: 'Крок 1', exam: 'Контроль', plan: 'План до модуля', guide: 'Як вчити', rules: 'Закономірності іннервації', bio: 'Медична біологія', facts: 'Теорія', micro: 'Мікроскоп' }[parts[0]] || '');
...
function renderAbout() {
  ...
  app.innerHTML = `<div class="wrap"><div class="prose">
    ...
    <h2>Контроль, план і закономірності</h2>
    <p>«Контроль» — репетиція практичного: одна структура без підписів, на час, 3D-моделі з випадкового ракурсу; помилки йдуть у повторення, у кінці — розбір. «План до модуля» розкладає невивчені теми по днях до дати контролю й перебудовується, якщо ви пропустили день. «Закономірності іннервації» — правила «група м’язів → нерв» із винятками й тестом; «З’єднай пари» — ще один спосіб пригадування для карток теорії. Коротка інструкція «Як вчити» — на «Сьогодні» і за адресою #/guide.</p>
    <h2>Медична біологія</h2>
    <p>Картки (клітина, спадкові хвороби, паразити), послідовності «розстав по порядку» (мітоз, мейоз, цикли паразитів), генератор генетичних задач із решіткою Пеннета, родоводи з розбором через спростування типів успадкування і «Мікроскоп» — впізнавання яєць гельмінтів, найпростіших і членистоногих за фото з ознаками.</p>
    <h2>3D-схеми</h2>
